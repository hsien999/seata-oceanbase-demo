...
class EscapeProcessor
{
    ...
    private static void processTimeToken(final MySQLConnection conn, final StringBuilder newSql, final String token) throws SQLException {
        final int startPos = token.indexOf(39) + 1;
        final int endPos = token.lastIndexOf(39);
        if (startPos == -1 || endPos == -1) {
            newSql.append(token);
            ...
            try {
                ...
                if (!conn.getUseTimezone() || !conn.getUseLegacyDatetimeCode()) {
                    ...
                    final Calendar sessionCalendar = conn.getCalendarInstanceForSessionOrNew();
                    try {
    ...
    private static void processTimestampToken(final MySQLConnection conn, final StringBuilder newSql, final String token) throws SQLException {
        final int startPos = token.indexOf(39) + 1;
        final int endPos = token.lastIndexOf(39);
        if (startPos == -1 || endPos == -1) {
            newSql.append(token);
    ...
    static {
        ...
        tempMap = new HashMap<String, String>(EscapeProcessor.JDBC_CONVERT_TO_MYSQL_TYPE_MAP);
        ...
        EscapeProcessor.JDBC_NO_CONVERT_TO_MYSQL_EXPRESSION_MAP = Collections.unmodifiableMap((Map<? extends String, ? extends String>)tempMap);
    }
}
