...
public abstract class ResultSetRow
{
    ...
    protected ComplexData[] complexRowData;
    ...
    public abstract ComplexData getComplexRowData(final int p0);
    
    public abstract byte[] getColumnValue(final int p0) throws SQLException;
    
    protected final Date getDateFast(final int columnIndex, final byte[] dateAsBytes, final int offset, int length, final MySQLConnection conn, final ResultSetImpl rs, final Calendar targetCalendar) throws SQLException {
        int year = 0;
        int month = 0;
        int day = 0;
        try {
            ...
            if (!onlyTimePresent && allZeroDate) {
                ...
                if (length >= 10) {
                    if (length != 18) {
                        ...
                        final StringTokenizer st = new StringTokenizer(StringUtils.toString(dateAsBytes, offset, length, "ISO8859_1"), "- ");
            ...
            final SQLException sqlEx2 = SQLError.createSQLException(Messages.getString("ResultSet.Bad_format_for_Date", new Object[] { StringUtils.toString(dateAsBytes), columnIndex + 1 }), "S1009", this.exceptionInterceptor);
            sqlEx2.initCause(e);
            throw sqlEx2;
        }
    }
    ...
    protected double getNativeDouble(final byte[] bits, final int offset) {
        final long valueAsLong = (long)(bits[offset + 0] & 0xFF) | (long)(bits[offset + 1] & 0xFF) << 8 | (long)(bits[offset + 2] & 0xFF) << 16 | (long)(bits[offset + 3] & 0xFF) << 24 | (long)(bits[offset + 4] & 0xFF) << 32 | (long)(bits[offset + 5] & 0xFF) << 40 | (long)(bits[offset + 6] & 0xFF) << 48 | (long)(bits[offset + 7] & 0xFF) << 56;
    ...
    protected Time getTimeFast(final int columnIndex, final byte[] timeAsBytes, final int offset, final int fullLength, final Calendar targetCalendar, final TimeZone tz, final boolean rollForward, final MySQLConnection conn, final ResultSetImpl rs) throws SQLException {
        int hr = 0;
        int min = 0;
        int sec = 0;
        int nanos = 0;
        ...
        try {
            ...
            if (!onlyTimePresent && allZeroTime) {
                ...
                if (decimalIndex != -1) {
                    ...
                    final int numDigits = fullLength - (decimalIndex + 1);
                    if (numDigits < 9) {
                ...
                if (timeColField.getMysqlType() == 7) {
                    ...
                    final SQLWarning sqlWarning = new SQLWarning(Messages.getString("ResultSet.Precision_lost_converting_TIMESTAMP_to_Time_with_getTime()_on_column__261") + columnIndex + "(" + timeColField + ").");
                }
                    ...
                    final SQLWarning sqlWarning2 = new SQLWarning(Messages.getString("ResultSet.Precision_lost_converting_DATETIME_to_Time_with_getTime()_on_column__264") + (columnIndex + 1) + "(" + timeColField + ").");
                }
                else {
                ...
                switch (length) {
                    ...
                    case 10: {
                        ...
                        for (int k = 0; k < length; ++k) {
                            if (timestampAsBytes[offset + k] == 45) {
                                hasDash = true;
                                break;
                            }
                        }
                    ...
                    case 8: {
                        ...
                        for (int k = 0; k < length; ++k) {
                            if (timestampAsBytes[offset + k] == 58) {
                                hasColon = true;
                                break;
                            }
                        }
