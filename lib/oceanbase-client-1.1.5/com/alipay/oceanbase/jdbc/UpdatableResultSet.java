...
public class UpdatableResultSet extends ResultSetImpl
{
    ...
    protected synchronized void generateStatements() throws SQLException {
        ...
        for (int i = 0; i < this.fields.length; ++i) {
            ...
            final StringBuilder fqcnBuf = new StringBuilder();
    ...
    public synchronized void moveToInsertRow() throws SQLException {
        ...
        (this.thisRow = new ByteArrayRow(newRowData, this.getExceptionInterceptor())).setMetadata(this.fields);
        for (int i = 0; i < numFields; ++i) {
            if (!this.populateInserterWithDefaultValues) {
    ...
    @Override
    public synchronized void realClose(final boolean calledExplicitly) throws SQLException {
        if (this.isClosed) {
            return;
        }
    ...
    protected synchronized void updateObjectInternal(final int columnIndex, final Object x, final Integer targetType, final int scaleOrLength) throws SQLException {
        if (!this.onInsertRow) {
            if (!this.doingUpdates) {
                this.doingUpdates = true;
                this.syncUpdate();
