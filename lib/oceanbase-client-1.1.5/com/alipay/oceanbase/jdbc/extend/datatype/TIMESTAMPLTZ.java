...
import com.alipay.oceanbase.jdbc.ConnectionImpl;
...
public class TIMESTAMPLTZ extends Datum
{
    ...
    public static String toString(final Connection connection, final byte[] bytes) throws SQLException {
        ...
        final Calendar calendar = Calendar.getInstance(TimeZone.getTimeZone(((ConnectionImpl)connection).getSessionTimeZone()));
        calendar.setTimeInMillis(TIMESTAMPTZ.getOriginTime(bytes, TimeZone.getTimeZone(((ConnectionImpl)connection).getSessionTimeZone())));
        ...
        return TIMESTAMPTZ.toString(year, month, day, hour, minute, second, nanos, bytes[11], Datum.getSessionTimeZone(connection));
    }
    ...
    public static Date toDate(final Connection connection, final byte[] bytes) throws SQLException {
        return new Date(TIMESTAMPTZ.getOriginTime(bytes, TimeZone.getTimeZone(((ConnectionImpl)connection).getSessionTimeZone())));
    }
    
    public static Time toTime(final Connection connection, final byte[] bytes) throws SQLException {
        return new Time(TIMESTAMPTZ.getOriginTime(bytes, TimeZone.getTimeZone(((ConnectionImpl)connection).getSessionTimeZone())));
    }
    ...
    public static Timestamp toTimestamp(final Connection connection, final byte[] bytes) throws SQLException {
        ...
        final Timestamp timestamp = new Timestamp(TIMESTAMPTZ.getOriginTime(bytes, TimeZone.getTimeZone(((ConnectionImpl)connection).getSessionTimeZone())));
    ...
    private static byte[] toBytes(final Connection connection, final Calendar calendar, final int nanos) throws SQLException {
        ...
        final SQLException exception = SQLError.createSQLException("error format", "268", null);
